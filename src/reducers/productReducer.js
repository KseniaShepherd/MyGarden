const LOAD_PRODUCTS = '[PRODUCTS_CONTAINER] LOAD PRODUCTS';
const LOAD_SALE_PRODUCTS = '[SALE_PRODUCTS_CONTAINER] LOAD SALE PRODUCTS';
const LOAD_THREE_SALE_PRODUCTS = '[SALE_SLIDER] LOAD_THREE_SALE_PRODUCTS'
const FILTER_PRODUCTS_BY_PRICE = '[PRODUCTS_CONTAINER] FILTER_PRODUCTS_BY_PRICE'
...
const getThreeRundomProducts = (payload) => {
    ...
    while (sliderState.length < iterations) {
        ...
        if (sliderState.indexOf(payload[randomNum]) === -1) sliderState.push(payload[randomNum]);
    }
    return sliderState.map(el => ({ ...el, visible: true, realPrice: getRealPrice(el.price, el.discont_price), discountPercentage: calculateDiscountPercentage(el.price, el.discont_price) }))
}
...
export const productReducer = (state = [], action) => {
    ...
    if (action.type === LOAD_PRODUCTS) {
        ...
        return action.payload.filter(el => el.discont_price > 0).map(el => ({ ...el, visible: true, realPrice: getRealPrice(el.price, el.discont_price), discountPercentage: calculateDiscountPercentage(el.price, el.discont_price) }))
        ...
        } else { return state.map(el => ({ ...el, visible: true })); }
