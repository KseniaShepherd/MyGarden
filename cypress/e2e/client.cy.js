describe('full purchase action', () => {
  ...
  it('posted order', () => {
    ...
    cy.contains('Thank you for the order! Your order has been processed. We will contact you shortly.')
  })
})
...
describe('submitting coupon request', () => {
  it('coupon request submitted', () => {
    ...
    cy.contains('Thank you for your request for a discount coupon. The coupon will be sent to the number you specified (1234567890). Expect a message shortly.')
...
describe('product filtering', () => {
  ...
  it('filtered by price', () => {
    ...
    cy.get('[data-testid="product-item"]').each((product) => {
      cy.wrap(product).find('[data-cy="productPrice"]').invoke('text').then(parseFloat).should('be.gte', 10);
      cy.wrap(product).find('[data-cy="productPrice"]').invoke('text').then(parseFloat).should('be.lte', 50);
    })
  })
  ...
  it('filtered by available discounted price', () => {
    ...
    cy.get('[data-testid="product-item"]').each((product) => {
      cy.wrap(product).find('[data-cy="discountPrice"]').invoke('text').then(parseFloat).should('be.greaterThan', 0)
    })
  });

  it('products are sorted in ascending order of price', () => {
    cy.get('[data-cy="sort-type"]').select("priceAscending");
    cy.get('[data-testid="product-item"]').should('have.length.at.least', 2).each((product, index, products) => {
      if (index < products.length - 1) {
        cy.wrap(product).find('[data-cy="productPrice"]').invoke('text').then(parseFloat).then(currentPrice => {
          cy.wrap(products[index + 1]).find('[data-cy="productPrice"]').invoke('text').then(parseFloat).then(nextPrice => {
  ...
  it('products are sorted in alphabetical order (A to Z)', () => {
    ...
    cy.get('[data-testid="product-item"]').should('have.length.at.least', 2).then((products) => {
      for (let i = 0; i < products.length - 1; i++) {
        const currentName = Cypress.$(products[i]).find('[data-cy="productName"]').text();
        const nextName = Cypress.$(products[i + 1]).find('[data-cy="productName"]').text();
